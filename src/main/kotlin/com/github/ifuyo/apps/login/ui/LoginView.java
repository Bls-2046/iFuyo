/*
 * Created by JFormDesigner on Mon Mar 03 12:10:28 CST 2025
 */

package com.github.ifuyo.apps.login.ui;

import com.github.ifuyo.apps.login.LoginController;

import java.awt.*;
import javax.swing.*;

/**
 * @author astar
 */
public class LoginView extends JFrame {
    public LoginView() {
        initComponents();
        // addLoginButtonListener();
         // setVerseLabel();
    }

//    private void setVerseLabel() {
//        String verse = Objects.requireNonNull(getVerseV1()).getContent(); // 获取诗句
//        String author = "—" + getVerseV1().getAuthor();
//        Verse.setText(verse); // 设置到 Verse 标签
//        Author.setText(author);
//    }

    private void initComponents() {
        // JFormDesigner - Component initialization - DO NOT MODIFY  //GEN-BEGIN:initComponents  @formatter:off
        title1 = new JLabel();
        loginButton = new JButton();
        username = new JLabel();
        usernameInput = new JTextField();
        password = new JLabel();
        passwordInput = new JPasswordField();
        title2 = new JLabel();
        Verse = new JLabel();
        separator1 = new JSeparator();
        separator2 = new JSeparator();
        Author = new JLabel();

        //======== this ========
        setIconImage(new ImageIcon(getClass().getResource("/Logo.png")).getImage());
        setTitle("iFuyo");
        setBackground(Color.orange);
        var contentPane = getContentPane();
        contentPane.setLayout(null);

        //---- title1 ----
        title1.setText("\u8f85\u4f51");
        title1.setFont(new Font("\u6c49\u4eea\u53f6\u53f6\u76f8\u601d\u4f53\u7b80", Font.PLAIN, 80));
        title1.setForeground(Color.darkGray);
        contentPane.add(title1);
        title1.setBounds(160, 15, title1.getPreferredSize().width, 90);

        //---- loginButton ----
        loginButton.setText("\u767b  \u5f55");
        loginButton.setFont(new Font("Microsoft YaHei UI", Font.PLAIN, 18));
        loginButton.setForeground(Color.darkGray);
        contentPane.add(loginButton);
        loginButton.setBounds(225, 290, 120, 43);

        //---- username ----
        username.setText("\u7528\u6237\u540d\uff1a");
        username.setFont(username.getFont().deriveFont(username.getFont().getStyle() & ~Font.BOLD, username.getFont().getSize() + 9f));
        username.setForeground(Color.darkGray);
        contentPane.add(username);
        username.setBounds(100, 164, 90, 40);

        //---- usernameInput ----
        usernameInput.setFont(usernameInput.getFont().deriveFont(usernameInput.getFont().getStyle() & ~Font.BOLD, usernameInput.getFont().getSize() + 5f));
        usernameInput.setHorizontalAlignment(SwingConstants.LEFT);
        contentPane.add(usernameInput);
        usernameInput.setBounds(185, 165, 260, 40);

        //---- password ----
        password.setText("\u5bc6\u7801\uff1a");
        password.setFont(password.getFont().deriveFont(password.getFont().getStyle() & ~Font.BOLD, password.getFont().getSize() + 8f));
        password.setForeground(Color.darkGray);
        contentPane.add(password);
        password.setBounds(110, 229, password.getPreferredSize().width, 40);

        //---- passwordInput ----
        passwordInput.setFont(passwordInput.getFont().deriveFont(passwordInput.getFont().getStyle() & ~Font.BOLD, passwordInput.getFont().getSize() + 5f));
        contentPane.add(passwordInput);
        passwordInput.setBounds(185, 230, 260, 40);

        //---- title2 ----
        title2.setText("Fuyo");
        title2.setFont(new Font("Yu Gothic UI", Font.BOLD, 40));
        title2.setForeground(new Color(0x666666));
        title2.setBackground(new Color(0xcccccc));
        contentPane.add(title2);
        title2.setBounds(new Rectangle(new Point(325, 25), title2.getPreferredSize()));

        //---- Verse ----
        Verse.setText("\u968f\u673a\u8bd7\u53e5");
        Verse.setFont(new Font("\u6977\u4f53", Font.BOLD, 18));
        Verse.setHorizontalAlignment(SwingConstants.CENTER);
        contentPane.add(Verse);
        Verse.setBounds(10, 105, 545, Verse.getPreferredSize().height);

        //---- separator1 ----
        separator1.setBackground(new Color(0x333333));
        separator1.setForeground(new Color(0x333333));
        contentPane.add(separator1);
        separator1.setBounds(130, 70, 300, 10);

        //---- separator2 ----
        separator2.setBackground(new Color(0x333333));
        separator2.setForeground(new Color(0x333333));
        contentPane.add(separator2);
        separator2.setBounds(315, 75, 100, 10);

        //---- Author ----
        Author.setText("\u4f5c\u8005");
        Author.setFont(new Font("\u6977\u4f53", Font.BOLD, 18));
        contentPane.add(Author);
        Author.setBounds(375, 130, 200, Author.getPreferredSize().height);

        contentPane.setPreferredSize(new Dimension(550, 360));
        pack();
        setLocationRelativeTo(getOwner());
        // JFormDesigner - End of component initialization  //GEN-END:initComponents  @formatter:on
    }
//    private void addLoginButtonListener() {
//        loginButton.addActionListener(e -> {
//            // 保存按钮的原始文本
//            String originalButtonText = loginButton.getText();
//
//            // 设置按钮为加载状态
//            loginButton.setText("登录中...");
//            loginButton.setEnabled(false); // 禁用按钮，防止重复点击
//
//            // 启动一个新线程执行耗时操作，避免阻塞 UI 线程
//            new Thread(() -> {
//                try {
//                    String username = usernameInput.getText();
//                    String password = new String(passwordInput.getPassword());
//
//                    // 调用 Python 脚本验证密码
//                    String pythonInterpreter = ".venv\\Scripts\\python.exe";
//                    String scriptPath = "src/main/resources/python/login_bitzh.py";
//
//                    // 启动 Python 进程
//                    Process process = Runtime.getRuntime().exec(new String[]{pythonInterpreter, scriptPath, username, password});
//
//                    // 读取 Python 脚本的标准输出
//                    BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream(), StandardCharsets.UTF_8));
//                    StringBuilder output = new StringBuilder();
//                    String line;
//                    while ((line = reader.readLine()) != null) {
//                        output.append(line);
//                    }
//                    String result = output.toString();
//                    System.out.println("Raw Python Output: " + result);
//
//                    // 解析 JSON 响应
//                    JSONObject jsonResponse = new JSONObject(result);
//                    if (jsonResponse.has("message")) {
//                        String message = jsonResponse.getString("message");
//                        if ("登录成功".equals(message)) {
//                            // 登录成功，显示消息并跳转到菜单界面
//                            SwingUtilities.invokeLater(() -> {
//                                JOptionPane.showMessageDialog(LoginView.this, "登录成功");
//                                dispose();
//                                new Profile(jsonResponse).setVisible(true);
//                            });
//                        } else {
//                            // 登录失败，显示错误消息
//                            SwingUtilities.invokeLater(() -> {
//                                JOptionPane.showMessageDialog(LoginView.this, message, "登录失败", JOptionPane.ERROR_MESSAGE);
//                            });
//                        }
//                    } else {
//                        // 未知错误
//                        SwingUtilities.invokeLater(() -> {
//                            JOptionPane.showMessageDialog(LoginView.this, "未知错误", "错误", JOptionPane.ERROR_MESSAGE);
//                        });
//                    }
//                } catch (Exception ex) {
//                    ex.printStackTrace();
//                    SwingUtilities.invokeLater(() -> {
//                        JOptionPane.showMessageDialog(LoginView.this, "登录失败，请重试", "错误", JOptionPane.ERROR_MESSAGE);
//                    });
//                } finally {
//                    // 恢复按钮的原始状态
//                    SwingUtilities.invokeLater(() -> {
//                        loginButton.setText(originalButtonText);
//                        loginButton.setEnabled(true);
//                    });
//
//                    // 清空输入框
//                    usernameInput.setText("");
//                    passwordInput.setText("");
//                }
//            }).start(); // 启动线程
//        });
//    }

    // Getter 方法
    public String getUsername() {
        return usernameInput.getText();
    }

    public String getPassword() {
        return new String(passwordInput.getPassword());
    }

    // Setter 方法
    public void setVerse(String verse) {
        Verse.setText(verse);
    }

    public void setAuthor(String author) {
        Author.setText(author);
    }

    // 获取登录按钮的文本
    public String getLoginButtonText() {
        return loginButton.getText();
    }

    // 设置登录按钮的文本
    public void setLoginButtonText(String text) {
        loginButton.setText(text);
    }

    // 设置登录按钮的启用状态
    public void setLoginButtonEnabled(boolean enabled) {
        loginButton.setEnabled(enabled);
    }

    // 绑定监听器
    public void LoginListener(LoginController listener) {
        loginButton.addActionListener(listener);
    }

    // 显示消息
    public void showMessage(String message) {
        JOptionPane.showMessageDialog(this, message);
    }

    // 显示错误消息
    public void showError(String message) {
        JOptionPane.showMessageDialog(this, message, "错误", JOptionPane.ERROR_MESSAGE);
    }

    // 清空输入框
    public void clearInputs() {
        usernameInput.setText("");
        passwordInput.setText("");
    }

    // JFormDesigner - Variables declaration - DO NOT MODIFY  //GEN-BEGIN:variables  @formatter:off
    private JLabel title1;
    private JButton loginButton;
    private JLabel username;
    private JTextField usernameInput;
    private JLabel password;
    private JPasswordField passwordInput;
    private JLabel title2;
    private JLabel Verse;
    private JSeparator separator1;
    private JSeparator separator2;
    private JLabel Author;
    // JFormDesigner - End of variables declaration  //GEN-END:variables  @formatter:on
}
